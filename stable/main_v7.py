...
class LiquidHandlerApp:
    ...
    def _build_combine_fractions_tab(self, parent):
        """Builds the 6-line combine fractions interface."""
        ...
        # Data structure to hold row variables
        self.combine_rows = []

        # Volume options: 100 to 1600 in steps of 100
        vol_options = [str(x) for x in range(100, 1700, 100)]

        # Default Falcon mapping for lines 1-6
        ...
        for i in range(6):
            ...
            # Source Start
            cb_start = ttk.Combobox(row_frame, textvariable=row_vars["start"], values=self.plate_wells, width=10,
                                    state="readonly")
            cb_start.pack(side="left", padx=2)

            # Source End
            cb_end = ttk.Combobox(row_frame, textvariable=row_vars["end"], values=self.plate_wells, width=10,
                                  state="readonly")
            cb_end.pack(side="left", padx=2)

            # Destination Falcon (with exclusivity logic)
            ...
            # Set Default Falcon if available
            if i < len(default_falcons):
                row_vars["dest"].set(default_falcons[i])

            # Bind event to update choices when selection changes
            cb_dest.bind("<<ComboboxSelected>>", lambda e: self._update_falcon_exclusivity())

            # Volume
            cb_vol = ttk.Combobox(row_frame, textvariable=row_vars["vol"], values=vol_options, width=8,
                                  state="readonly")
            cb_vol.pack(side="left", padx=2)

            # Store widgets and vars
            self.combine_rows.append({
                "vars": row_vars,
                "widgets": {"dest": cb_dest}
            })

        # Initial update of exclusivity
        ...
        ttk.Label(btn_frame, text="Logic: Eject -> Pick -> Transfer Range (Split >800uL) -> Eject. Repeats per line.",
                  font=("Arial", 8, "italic")).pack(pady=5)

    def _update_falcon_exclusivity(self):
        """Ensures a Falcon selected in one line is hidden in others."""
        ...
        for row in self.combine_rows:
            ...
            if val:
                selected_falcons.add(val)

        # Update each combobox
        for row in self.combine_rows:
            current_val = row["vars"]["dest"].get()
            # Available = All - Selected + Current (so we keep the current one in the list)
    ...
    # ==========================================
    #           COMBINE FRACTIONS LOGIC
    # ==========================================

    def combine_fractions_sequence(self):
        """
        Iterates through the 6 lines defined in the Combine Fractions tab.
        For each line:
        1. Eject current tip (to ensure fresh start).
        2. Pick new tip.
        3. Iterate through Source Wells (Start to End).
        4. Transfer Volume (Splitting into 800uL batches if needed).
        5. Eject tip after line is done.
        """
        if not self.ser or not self.ser.is_open:
            messagebox.showwarning("Not Connected", "Please connect to the printer first.")
            return
        ...
        # Air Gap / Buffer Logic
        # We keep 200uL air gap.
        # Max transfer per stroke = 800uL.
        # E position logic: 0 is home. -200 is air gap ready. -(200+Vol) is full.

        air_gap_vol = 200.0
        e_pos_air_gap = -1 * air_gap_vol * STEPS_PER_UL

        # Blowout target: Eject amount + 100.
        # If we are at -(200+Vol), and we eject Vol, we are at -200.
        # Then eject +100 -> we are at -100.
        ...
        for task in tasks:
            ...
            # 1. Eject Tip (Ensure clean start)
            ...
            for well in wells:
                ...
                # Calculate batches (Max 800uL per stroke)
                # If vol is 1200, we do 600 + 600.
                ...
                for b in range(num_batches):
                    ...
                    # Calculate E positions for this batch
                    vol_aspirated = batch_vol
                    e_pos_full = -1 * (air_gap_vol + vol_aspirated) * STEPS_PER_UL

                    cmds = []

                    # A. Prepare Pipette (Move to Air Gap Position)
                    cmds.append(f"G1 E{e_pos_air_gap:.3f} F{PIP_SPEED}")

                    # B. Move to Source Well
                    sx, sy = self.get_well_coordinates(well)
                    # Smart travel logic
                    if self.last_known_module == "PLATE":
                        cmds.append(f"G0 Z{plate_safe_z:.2f} F{JOG_SPEED_Z}")
                        cmds.append(f"G0 X{sx:.2f} Y{sy:.2f} F{JOG_SPEED_XY}")
                    else:
                        # Go to global safe, then plate safe
                        # (Simplified here assuming smart_travel helper logic inline)
                        global_safe_z = self.resolve_coords(0, 0, GLOBAL_SAFE_Z_OFFSET)[2]
                        cmds.append(f"G0 Z{global_safe_z:.2f} F{JOG_SPEED_Z}")
                        cmds.append(f"G0 X{sx:.2f} Y{sy:.2f} F{JOG_SPEED_XY}")
                        cmds.append(f"G0 Z{plate_safe_z:.2f} F{JOG_SPEED_Z}")
                    ...
                    # Update module tracking for next loop iteration optimization
                    self.update_last_module("PLATE")

                    # D. Move to Destination Falcon
                    dx, dy = self.get_falcon_coordinates(dest_falcon)
                    # Travel from Plate Safe to Falcon Safe
                    cmds.append(f"G0 Z{plate_safe_z:.2f} F{JOG_SPEED_Z}")  # Ensure up
                    cmds.append(f"G0 X{dx:.2f} Y{dy:.2f} F{JOG_SPEED_XY}")
                    cmds.append(f"G0 Z{falcon_safe_z:.2f} F{JOG_SPEED_Z}")

                    # E. Dispense (with blowout)
                    cmds.append(f"G0 Z{falcon_disp_z:.2f} F{JOG_SPEED_Z}")
                    # Dispense amount + 100uL (blowout)
                    ...
                    # Update Logical Volume (we are at roughly 100uL physical pos)
                    self.current_pipette_volume = 100.0
                    self.vol_display_var.set(f"{self.current_pipette_volume:.1f} uL")
                    self.live_vol_var.set(f"{self.current_pipette_volume:.1f}")

            # 4. Eject Tip after line is complete
