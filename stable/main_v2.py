...
FALCON_RACK_CONFIG = {
    ...
    "Z_SAFE": 195.0,
    "Z_ASPIRATE": 80.0, "Z_DISPENSE": 110.0
}
...
FILTER_EPPI_RACK_CONFIG = {
    "A1_X": 160.05, "A1_Y": 53.5,
    "B2_X": 194.05, "B2_Y": 16.5,
    "Z_SAFE": 195.0,
    "Z_ASPIRATE": 80.0, "Z_DISPENSE": 110.0
}
...
class LiquidHandlerApp:
    def __init__(self, root):
        ...
        # 6-10. Generic 2x2 Racks
        self.generic_2x2_positions = ["A1", "A2", "B1", "B2"]
        ...
        self.modules = {
            ...
            "FILTER_EPPI": {
                "label": "Filter Eppi", "var": tk.StringVar(), "values": self.generic_2x2_positions,
                "btn_text": "GO",
                "cmd": lambda: self.generic_move_sequence("FILTER_EPPI", self.modules["FILTER_EPPI"]["var"].get())
            },
            "EPPI": {
                "label": "Eppi Rack", "var": tk.StringVar(), "values": self.generic_2x2_positions,
                "btn_text": "GO", "cmd": lambda: self.generic_move_sequence("EPPI", self.modules["EPPI"]["var"].get())
            },
            "HPLC": {
                "label": "HPLC Vial", "var": tk.StringVar(), "values": self.generic_2x2_positions,
                "btn_text": "GO", "cmd": lambda: self.generic_move_sequence("HPLC", self.modules["HPLC"]["var"].get())
            },
            "HPLC_INSERT": {
                "label": "HPLC Insert", "var": tk.StringVar(), "values": self.generic_2x2_positions,
                "btn_text": "GO",
                "cmd": lambda: self.generic_move_sequence("HPLC_INSERT", self.modules["HPLC_INSERT"]["var"].get())
            },
            "SCREWCAP": {
                "label": "Screwcap Vial", "var": tk.StringVar(), "values": self.generic_2x2_positions,
                "btn_text": "GO",
    ...
    def _build_pipette_tab(self, parent):
        ...
        ttk.Button(pip_frame, text="EJECT TIP", command=self.eject_tip_sequence).pack(fill="x", pady=2)
    ...
    def get_generic_2x2_coordinates(self, key, config):
        # Assumes A1 to B2 (2 rows, 2 cols)
        row_char = key[0]
        col_num = int(key[1:])
        rows = ["A", "B"]
        if row_char not in rows: return 0.0, 0.0
        row_idx = rows.index(row_char)
        col_idx = col_num - 1
        return self._get_interpolated_coords(col_idx, row_idx, 2, 2, config["A1_X"], config["A1_Y"], config["B2_X"],
                                             config["B2_Y"])
    ...
    def eject_tip_sequence(self):
        ...
        commands.extend([f"G0 Z160.2 F{JOG_SPEED_Z}", f"G4 P500", f"G0 Y218 F250", f"G0 Z190 F250",
    ...
    def pick_tip_sequence(self):
        ...
        commands.extend([f"G0 Z{pick_z} F500", "G4 P500", f"G0 Z{rack_safe_z} F{JOG_SPEED_Z}"])
    ...
    def generic_move_sequence(self, module_name, target_pos):
        ...
        if module_name == "PLATE":
            ...
            x, y = self.get_generic_2x2_coordinates(target_pos, FILTER_EPPI_RACK_CONFIG)
            safe_z = FILTER_EPPI_RACK_CONFIG["Z_SAFE"]
        elif module_name == "EPPI":
            x, y = self.get_generic_2x2_coordinates(target_pos, EPPI_RACK_CONFIG)
            safe_z = EPPI_RACK_CONFIG["Z_SAFE"]
        elif module_name == "HPLC":
            x, y = self.get_generic_2x2_coordinates(target_pos, HPLC_VIAL_RACK_CONFIG)
            safe_z = HPLC_VIAL_RACK_CONFIG["Z_SAFE"]
        elif module_name == "HPLC_INSERT":
            x, y = self.get_generic_2x2_coordinates(target_pos, HPLC_VIAL_INSERT_RACK_CONFIG)
            safe_z = HPLC_VIAL_INSERT_RACK_CONFIG["Z_SAFE"]
        elif module_name == "SCREWCAP":
            x, y = self.get_generic_2x2_coordinates(target_pos, SCREWCAP_VIAL_RACK_CONFIG)
