...
VOLATILE_DRIFT_RATE = 200
VOLATILE_MOVE_SPEED = 200
...
class LiquidHandlerApp:
    ...
    def _perform_single_transfer(self, source_str, dest_str, vol, is_volatile, e_gap_pos, air_gap_ul,
        ...
        # --- VOLATILE MIXING LOGIC (ADDED) ---
        if is_volatile:
            # The prompt requests mixing 3 times before aspirating the actual volume
            # to saturate the headspace.
            # Logic: Aspirate a fixed amount (e.g. 500uL) then dispense back to air gap.
            mix_cycles = 3
            mix_vol = 500.0  # Good amount to saturate tip without hitting max if air gap is 200
            # Ensure we don't exceed max pipette vol (1000)
            # Current is air_gap_ul (200). 200 + 500 = 700 < 1000. Safe.

            e_mix_down = -1 * (air_gap_ul + mix_vol) * STEPS_PER_UL
            e_mix_up = -1 * (air_gap_ul) * STEPS_PER_UL  # Back to starting gap

            self.log_line(f"[VOLATILE] Pre-wetting/Mixing source 3 times...")

            for _ in range(mix_cycles):
                cmds.append(f"G1 E{e_mix_down:.3f} F{PIP_SPEED}")  # Suck
                cmds.append(f"G1 E{e_mix_up:.3f} F{PIP_SPEED}")    # Spit back
        # -------------------------------------

        cmds.append(f"G1 E{e_loaded_pos:.3f} F{PIP_SPEED}")

        # --- VOLATILE LOGIC FOR LIFT OUT (keeps sucking while lifting) ---
        if is_volatile:
            ...
            e_loaded_pos = e_lifted_pos  # update baseline for travel
        else:
            cmds.append(f"G0 Z{src_safe_z:.2f} F{JOG_SPEED_Z}")
        ...
        # 3. Move to Dest (With Volatile Logic: suck during Z up, XY, and Z down to safe Z)
        cmds_move = []
        if is_volatile:
            ...
            self.log_line(
                f"[VOLATILE] Drift (Up+XY+DownSafe): {drift_steps_up + drift_steps_xy + drift_steps_down:.1f} steps")
            volatile_current_e = current_e  # carry forward for dispense Z move
        else:
            ...
            volatile_current_e = e_loaded_pos  # no extra drift in non-volatile case

        self._send_lines_with_ok(cmds_move)
        self.update_last_module(dest_mod)
        current_mod_tracker = dest_mod

        # 4. Dispense (with volatile suction on final Z down into destination)
        e_blowout_pos = -1 * 100.0 * STEPS_PER_UL
        cmds_disp = []

        if is_volatile:
            dist_z_disp = abs(dest_safe_z - dest_disp_z)
            time_z_disp = dist_z_disp / JOG_SPEED_Z
            drift_steps_disp = (VOLATILE_DRIFT_RATE * STEPS_PER_UL) * (time_z_disp / 60.0)
            volatile_current_e -= drift_steps_disp
            cmds_disp.append(f"G1 Z{dest_disp_z:.2f} E{volatile_current_e:.3f} F{JOG_SPEED_Z}")
            self.log_line(f"[VOLATILE] Drift (Down to Disp): {drift_steps_disp:.1f} steps")
        else:
