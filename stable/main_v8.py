...
# Volatile Logic
# Rate of aspiration (units/min) during XY travel
...
class LiquidHandlerApp:
    ...
    def _build_transfer_liquid_tab(self, parent):
        ...
        # UPDATED: Added "Execute" column
        cols = [
            ("Execute", 8), ("Line", 4), ("Source Vial", 15), ("Dest Vial", 15),
            ("Vol (uL)", 8), ("Volatile", 8),
            ("Wash Vol", 8), ("Wash Source", 12)
        ]
        ...
        wash_vol_options = ["0"] + [str(x) for x in range(100, 1700, 100)]

        self.transfer_rows = []

        for i in range(8):
            row_vars = {
                "execute": tk.BooleanVar(value=False),  # Default OFF
            ...
            ttk.Combobox(row_frame, textvariable=row_vars["vol"], values=vol_options, width=8, state="readonly").pack(
                side="left", padx=2)
            ttk.Checkbutton(row_frame, variable=row_vars["volatile"], text="Volatile", width=10).pack(side="left",
                                                                                                      padx=2)
    ...
    def _build_combine_fractions_tab(self, parent):
        ...
        header_frame = ttk.Frame(frame)
        header_frame.pack(fill="x", pady=(0, 5))
        ttk.Label(header_frame, text="Line", width=6, font=("Arial", 9, "bold")).grid(row=0, column=0)
        ttk.Label(header_frame, text="Source Start", width=12, font=("Arial", 9, "bold")).grid(row=0, column=1)
        ttk.Label(header_frame, text="Source End", width=12, font=("Arial", 9, "bold")).grid(row=0, column=2)
        ttk.Label(header_frame, text="Dest Falcon", width=12, font=("Arial", 9, "bold")).grid(row=0, column=3)
        ttk.Label(header_frame, text="Vol (uL)", width=10, font=("Arial", 9, "bold")).grid(row=0, column=4)
    ...
    def transfer_liquid_sequence(self):
        ...
        def run_seq():
            ...
            for task in tasks:
                ...
                if wash_vol > 0:
                    self.log_line(f"[L{line_num}] Starting Wash Sequence ({wash_vol}uL)...")
                    if wash_vol > 800:
                        cycle_vol = wash_vol / 2.0
                        self.log_line(f"[L{line_num}] Wash > 800uL. Splitting into 2 cycles of {cycle_vol}uL.")
                        current_simulated_module = self._perform_wash_cycle(
                            task["wash_src"], task["source"], task["dest"], cycle_vol, is_volatile,
    ...
    def combine_fractions_sequence(self):
        ...
        if not tasks:
            messagebox.showinfo("No Tasks", "No valid lines configured.")
            return
        ...
        for task in tasks:
            ...
            for well in wells:
                ...
                for b in range(num_batches):
                    ...
                    self.vol_display_var.set(f"{self.current_pipette_volume:.1f} uL")
                    self.live_vol_var.set(f"{self.current_pipette_volume:.1f}")
            self.log_line(f"[COMBINE] Line {line_num} Complete. Ejecting...")
            self._send_lines_with_ok(self._get_eject_tip_commands())
            self.update_last_module("EJECT")
