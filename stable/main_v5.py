...
FALCON_RACK_CONFIG = {
    "15ML_A1_X": 12.8, "15ML_A1_Y": 129.2,
    "15ML_B3_X": 50.3, "15ML_B3_Y": 100.7,
    "50ML_X": 74.7, "50ML_Y": 119.8,
    "Z_SAFE": 195.0,
...
class LiquidHandlerApp:
    def __init__(self, root):
        ...
        self.root.geometry("1024x600")  # Increased height for visual map
        ...
        # Visual Distribution Variables
        self.dist_volume_var = tk.StringVar(value="100")
        self.dist_source_var = tk.StringVar(value="Wash A")
        self.selected_targets = []  # List of tuples (module_name, position)
        self.canvas_hit_zones = []  # List of (x, y, r, module, pos, item_id)
    ...
    def _build_tip_tab(self, parent):
        # Create a scrollable frame for Maintenance because the visual map is large
        ...
        ttk.Label(scrollable_frame, text="Maintenance Mode", font=("Arial", 16, "bold")).pack(pady=(10, 10))

        # General Controls
        ctrl_frame = ttk.Frame(scrollable_frame)
        ctrl_frame.pack(fill="x", padx=10)
        ttk.Button(ctrl_frame, text="Park Head (Safe)", command=lambda: self.send_home("XY")).pack(fill="x", pady=2)
        ttk.Button(ctrl_frame, text="UNPAUSE / RESUME (M108)", command=self.send_resume).pack(fill="x", pady=2)

        # --- VISUAL DISTRIBUTION INTERFACE ---
        vis_frame = ttk.LabelFrame(scrollable_frame, text="Visual Distribution Test", padding=10)
        vis_frame.pack(fill="both", expand=True, padx=10, pady=10)

        # Settings Row
        settings_row = ttk.Frame(vis_frame)
        settings_row.pack(fill="x", pady=(0, 10))

        # Volume Combobox
        ttk.Label(settings_row, text="Volume (uL):").pack(side="left", padx=5)
        vol_values = [str(x) for x in range(100, 900, 100)]  # 100, 200 ... 800
        self.vol_combo = ttk.Combobox(settings_row, textvariable=self.dist_volume_var, values=vol_values, width=5,
                                      state="readonly")
        self.vol_combo.pack(side="left", padx=5)

        # Source Combobox
        ttk.Label(settings_row, text="Source:").pack(side="left", padx=5)
        # Combine Falcon and Wash positions for source
        source_values = ["Wash A"] + self.wash_positions + self.falcon_positions
        # Remove duplicates if any
        source_values = list(dict.fromkeys(source_values))
        self.source_combo = ttk.Combobox(settings_row, textvariable=self.dist_source_var, values=source_values,
                                         width=12, state="readonly")
        self.source_combo.pack(side="left", padx=5)

        # Run Button
        ttk.Button(settings_row, text="RUN DISTRIBUTION", command=self.run_visual_distribution_sequence).pack(
            side="right", padx=5)
        ttk.Button(settings_row, text="Clear Selection", command=self.clear_visual_selection).pack(side="right", padx=5)

        # Canvas for Module Map
        self.map_canvas = tk.Canvas(vis_frame, width=450, height=450, bg="#f0f0f0", relief="sunken", borderwidth=1)
        ...
        # --- EXISTING TESTS ---
        test_frame = ttk.LabelFrame(scrollable_frame, text="Legacy Module Tests", padding=10)
        test_frame.pack(fill="x", padx=10, pady=10)

        ttk.Button(test_frame, text="TEST: Rack Tip Module (Random Pick/Eject)",
    ...
    def draw_module_map(self):
        ...
        self.canvas_hit_zones = []

        # Coordinate transformation:
        # Machine X: ~ -10 to 220
        # Machine Y: ~ -10 to 220
        # Canvas: 450x450
        # We need to flip Y because Canvas Y=0 is top, Machine Y=0 is bottom (usually),
        # but looking at config, Plate A1 is Y=218 (Top/Back) and Screwcap F1 is Y=-8 (Front).
        # So Machine Y High = Top. Canvas Y Low = Top.
        # Scale factor:
        scale = 1.6
        offset_x = 40
        offset_y = 20  # Padding from top

        # Helper to transform
        def to_screen(mx, my):
            sx = (mx * scale) + offset_x
            # Invert Y: Max Y is approx 220.
            sy = ((230 - my) * scale) + offset_y
            return sx, sy

        def draw_circle(mx, my, radius, color, module, pos):
            sx, sy = to_screen(mx, my)
            # Check if selected
            is_selected = (module, pos) in self.selected_targets
            fill_color = "#00FF00" if is_selected else color
            outline = "black" if is_selected else "gray"

            item_id = self.map_canvas.create_oval(sx - radius, sy - radius, sx + radius, sy + radius, fill=fill_color,
                                                  outline=outline)
            self.canvas_hit_zones.append((sx, sy, radius, module, pos, item_id))

            # Text label for corners
            if pos in ["A1", "H12", "1", "8"]:
                self.map_canvas.create_text(sx, sy, text=pos, font=("Arial", 6), fill="black")

        # 1. Draw Plate
        for pos in self.plate_wells:
            x, y = self.get_well_coordinates(pos)
            draw_circle(x, y, 4, "white", "PLATE", pos)

        # Label Plate
        px, py = to_screen(PLATE_CONFIG["A1_X"], PLATE_CONFIG["A1_Y"])
        self.map_canvas.create_text(px, py - 15, text="96 Plate", font=("Arial", 8, "bold"))

        # 2. Draw Falcon
        for pos in self.falcon_positions:
            x, y = self.get_falcon_coordinates(pos)
            r = 8 if pos == "50mL" else 6
            draw_circle(x, y, r, "#e0e0ff", "FALCON", pos)

        # 3. Draw Wash
        for pos in self.wash_positions:
            x, y = self.get_wash_coordinates(pos)
            draw_circle(x, y, 8, "#ffe0e0", "WASH", pos)

        # 4. Draw Tube Racks (Rows A-F)
        # 4mL
        for pos in self._4ml_positions:
            x, y = self.get_4ml_coordinates(pos)
            draw_circle(x, y, 5, "#ffffcc", "4ML", pos)

        # Filter Eppi
        for pos in self.filter_eppi_positions:
            x, y = self.get_1x8_rack_coordinates(pos, FILTER_EPPI_RACK_CONFIG, "B")
            draw_circle(x, y, 4, "#ccffcc", "FILTER_EPPI", pos)

        # Eppi
        for pos in self.eppi_positions:
            x, y = self.get_1x8_rack_coordinates(pos, EPPI_RACK_CONFIG, "C")
            draw_circle(x, y, 4, "#ccffff", "EPPI", pos)

        # HPLC
        for pos in self.hplc_positions:
            x, y = self.get_1x8_rack_coordinates(pos, HPLC_VIAL_RACK_CONFIG, "D")
            draw_circle(x, y, 4, "#f0ccff", "HPLC", pos)

        # HPLC Insert
        for pos in self.hplc_insert_positions:
            x, y = self.get_1x8_rack_coordinates(pos, HPLC_VIAL_INSERT_RACK_CONFIG, "E")
            draw_circle(x, y, 3, "#ffccf0", "HPLC_INSERT", pos)

        # Screwcap
        for pos in self.screwcap_positions:
            x, y = self.get_1x8_rack_coordinates(pos, SCREWCAP_VIAL_RACK_CONFIG, "F")
            draw_circle(x, y, 4, "#cccccc", "SCREWCAP", pos)

        # Draw Tips (Visual Only - Not selectable targets usually, but good for reference)
        # We won't make them clickable targets to avoid confusion with picking tips
        for r in self.tip_rows:
            for c in self.tip_cols:
                key = f"{r}{c}"
                x, y = self.get_tip_coordinates(key)
                sx, sy = to_screen(x, y)
                color = "#90ee90" if self.tip_inventory[key] else "#ffcccb"
                self.map_canvas.create_oval(sx - 4, sy - 4, sx + 4, sy + 4, fill=color, outline="gray")

        tx, ty = to_screen(TIP_RACK_CONFIG["A1_X"], TIP_RACK_CONFIG["A1_Y"])
        self.map_canvas.create_text(tx + 20, ty - 10, text="Tips", font=("Arial", 8, "bold"))

    def on_canvas_click(self, event):
        cx, cy = event.x, event.y
        clicked_something = False

        for (sx, sy, r, module, pos, item_id) in self.canvas_hit_zones:
            # Simple distance check
            dist = math.sqrt((cx - sx) ** 2 + (cy - sy) ** 2)
            if dist <= r + 2:  # +2 tolerance
                target = (module, pos)
        ...
        if clicked_something:
            self.draw_module_map()  # Redraw to update colors

    def clear_visual_selection(self):
        self.selected_targets = []
        self.draw_module_map()

    def run_visual_distribution_sequence(self):
        """
        1. Find new tip.
        2. For each selected target:
           - Asp volume from Source.
           - Disp volume to Target.
        3. Eject Tip.
        """
        ...
        if not self.selected_targets:
            messagebox.showwarning("No Targets", "Please click on vials/wells in the map to select targets.")
            return

        try:
            vol_ul = float(self.dist_volume_var.get())
        except ValueError:
            messagebox.showerror("Invalid Volume", "Check volume input.")
            return

        source_name = self.dist_source_var.get()

        # Determine Source Module and Position from the name
        source_module = ""
        source_pos = source_name

        if "Wash" in source_name or "Waste" in source_name:
            source_module = "WASH"
        elif source_name in self.falcon_positions:
            source_module = "FALCON"
        else:
            # Fallback logic if user selected something weird, though combobox restricts this
            source_module = "WASH"

            # Find Tip
        tip_key = self._find_next_available_tip()
        if not tip_key:
            messagebox.showerror("No Tips", "No fresh tips available in inventory.")
            return

        self.log_line(f"[DIST] Starting Distribution. Source: {source_name}, Vol: {vol_ul}uL")
        self.log_line(f"[DIST] Targets: {len(self.selected_targets)}")
        ...
        def run_seq():
            ...
            self.root.after(0, self.draw_module_map)  # Update map tip colors

            # 2. Loop Targets
            # We use a helper to transfer one by one
            for (tgt_mod, tgt_pos) in self.selected_targets:
                self.log_line(f"[DIST] Transferring to {tgt_mod} {tgt_pos}...")

                # --- ASPIRATE FROM SOURCE ---
                # Get Source Coords & Config
                sx, sy = 0, 0
                scfg = {}
                ...
                # Ensure pipette is reset (Air Gap logic or just reset E)
                # We assume we start empty or with air gap.
                # To be safe, let's assume we want to aspirate 'vol_ul' into the tip.
                # Reset E to 0 for simplicity in this loop relative moves, or track absolute.
                # Let's use Absolute E tracking.

                # Move to Source
                cmds_asp = self._get_smart_travel_gcode(source_module, sx, sy, scfg["Z_SAFE"])

                # Calculate E
                # Current vol is whatever it is. We want to add vol_ul.
                target_vol_asp = self.current_pipette_volume + vol_ul
                ...
                self.root.after(0, lambda v=target_vol_asp: self.vol_display_var.set(f"{v:.1f} uL"))
                ...
                if tgt_mod == "PLATE":
                    tx, ty = self.get_well_coordinates(tgt_pos)
                    tcfg = PLATE_CONFIG
                elif tgt_mod == "FALCON":
                    tx, ty = self.get_falcon_coordinates(tgt_pos)
                    tcfg = FALCON_RACK_CONFIG
                elif tgt_mod == "WASH":
                    tx, ty = self.get_wash_coordinates(tgt_pos)
                    tcfg = WASH_RACK_CONFIG
                elif tgt_mod == "4ML":
                    tx, ty = self.get_4ml_coordinates(tgt_pos)
                    tcfg = _4ML_RACK_CONFIG
                elif tgt_mod == "FILTER_EPPI":
                    tx, ty = self.get_1x8_rack_coordinates(tgt_pos, FILTER_EPPI_RACK_CONFIG, "B")
                    tcfg = FILTER_EPPI_RACK_CONFIG
                elif tgt_mod == "EPPI":
                    tx, ty = self.get_1x8_rack_coordinates(tgt_pos, EPPI_RACK_CONFIG, "C")
                    tcfg = EPPI_RACK_CONFIG
                elif tgt_mod == "HPLC":
                    tx, ty = self.get_1x8_rack_coordinates(tgt_pos, HPLC_VIAL_RACK_CONFIG, "D")
                    tcfg = HPLC_VIAL_RACK_CONFIG
                elif tgt_mod == "HPLC_INSERT":
                    tx, ty = self.get_1x8_rack_coordinates(tgt_pos, HPLC_VIAL_INSERT_RACK_CONFIG, "E")
                    tcfg = HPLC_VIAL_INSERT_RACK_CONFIG
                elif tgt_mod == "SCREWCAP":
                    tx, ty = self.get_1x8_rack_coordinates(tgt_pos, SCREWCAP_VIAL_RACK_CONFIG, "F")
                    tcfg = SCREWCAP_VIAL_RACK_CONFIG

                # Move to Target
                cmds_disp = self._get_smart_travel_gcode(tgt_mod, tx, ty, tcfg["Z_SAFE"])

                # Calculate E (Dispense back)
                target_vol_disp = self.current_pipette_volume - vol_ul
                target_e_disp = -1 * target_vol_disp * STEPS_PER_UL

                cmds_disp.extend([
                    f"G0 Z{tcfg['Z_DISPENSE']} F{JOG_SPEED_Z}",
                    f"G1 E{target_e_disp:.3f} F{PIP_SPEED}",
                    f"G0 Z{tcfg['Z_SAFE']} F{JOG_SPEED_Z}"
                ])
                self._send_lines_with_ok(cmds_disp)
                self.current_pipette_volume = target_vol_disp
                self.last_known_module = tgt_mod
                self.root.after(0, lambda v=target_vol_disp: self.vol_display_var.set(f"{v:.1f} uL"))
            ...
            self.log_line("[DIST] Sequence Complete.")
            self.root.after(0, self.clear_visual_selection)
    ...
    def _get_eject_tip_commands(self):
        ...
        # 1. Approach
        commands = self._get_smart_travel_gcode("EJECT", cfg["APPROACH_X"], cfg["APPROACH_Y"], cfg["Z_SAFE"])

        # 2. Ejection Sequence
        # Move down to engage
        commands.append(f"G0 Z{cfg['Z_EJECT_START']} F{JOG_SPEED_Z}")
        # Move Y to strip tip
        commands.append(f"G0 Y{cfg['EJECT_TARGET_Y']} F250")
        # Move Z up
        commands.append(f"G0 Z{cfg['Z_RETRACT']} F250")
        # Return to Global Safe Z
        commands.append(f"G0 Z{GLOBAL_SAFE_Z} F{JOG_SPEED_Z}")
    ...
    def pick_tip_sequence(self):
        ...
        def run_seq():
            ...
            self.root.after(0, self.draw_module_map)
