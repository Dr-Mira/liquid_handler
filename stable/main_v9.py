...
class LiquidHandlerApp:
    ...
    def _build_transfer_liquid_tab(self, parent):
        ...
        # UPDATED: Added "Wash Times" column
        cols = [
            ("Execute", 8), ("Line", 4), ("Source Vial", 15), ("Dest Vial", 15),
            ("Vol (uL)", 8), ("Volatile", 8),
        ...
        source_options = []
        source_options.extend([f"4mL {p}" for p in self._4ml_positions])
        source_options.extend([f"Falcon {p}" for p in self.falcon_positions])
        source_options.extend([f"Filter Eppi {p}" for p in self.filter_eppi_positions])
        source_options.extend([f"Eppi {p}" for p in self.eppi_positions])
        source_options.extend([f"HPLC {p}" for p in self.hplc_positions])
        source_options.extend([f"HPLC Insert {p}" for p in self.hplc_insert_positions])
        source_options.extend([f"Screwcap {p}" for p in self.screwcap_positions])
        source_options.extend(self.wash_positions)

        vol_options = [str(x) for x in range(100, 1700, 100)]

        # Default Wash Volume options (100-800)
        wash_vol_options_std = ["0"] + [str(x) for x in range(100, 900, 100)]
        # Volatile Wash Volume options
        ...
        for i in range(8):
            row_vars = {
                ...
                "source": tk.StringVar(value="4mL A1"),
            ...
            ttk.Combobox(row_frame, textvariable=row_vars["source"], values=source_options, width=15,
                         state="readonly").pack(side="left", padx=2)
            ttk.Combobox(row_frame, textvariable=row_vars["dest"], values=source_options, width=15,
                         state="readonly").pack(side="left", padx=2)
            ...
            # Wash Volume Combobox (needs reference to update values)
            cb_wash_vol = ttk.Combobox(row_frame, textvariable=row_vars["wash_vol"], values=wash_vol_options_std,
                                       width=8,
                                       state="readonly")
            cb_wash_vol.pack(side="left", padx=2)

            # Callback to update Wash Volume options based on Volatile state
        ...
        ttk.Button(btn_frame, text="EXECUTE TRANSFER SEQUENCE",
    ...
    def park_head_sequence(self):
        ...
        commands = [
            "G90",
            f"G0 Z{abs_safe_z:.2f} F{JOG_SPEED_Z}",
            f"G0 X{abs_center_x:.2f} Y{abs_center_y:.2f} F{JOG_SPEED_XY}",
    ...
    def _perform_single_transfer(self, source_str, dest_str, vol, is_volatile, e_gap_pos, air_gap_ul,
        ...
        # --- Z OPTIMIZATION LOGIC FOR SOURCE ---
        use_optimized_z_src = (current_mod_tracker in SMALL_VIAL_MODULES and src_mod in SMALL_VIAL_MODULES)
        if use_optimized_z_src:
            travel_z_src = self.resolve_coords(0, 0, _4ML_RACK_CONFIG["Z_SAFE"])[2]
        else:
            travel_z_src = global_safe_z
        ...
        # --- VOLATILE LOGIC FOR LIFT OUT (FIXED) ---
        if is_volatile:
            # Calculate drift for the lift out of the vial (Z_ASP -> Z_SAFE)
            dist_lift = abs(src_safe_z - src_asp_z)
            time_lift = dist_lift / JOG_SPEED_Z
            drift_lift = (VOLATILE_DRIFT_RATE * STEPS_PER_UL) * (time_lift / 60.0)

            # Retract more (negative E) while lifting
            e_lifted_pos = e_loaded_pos - drift_lift

            # Use G1 for synchronized Z and E movement
            cmds.append(f"G1 Z{src_safe_z:.2f} E{e_lifted_pos:.3f} F{JOG_SPEED_Z}")

            # Update the baseline E position for the next travel phase
            e_loaded_pos = e_lifted_pos
        else:
            cmds.append(f"G0 Z{src_safe_z:.2f} F{JOG_SPEED_Z}")

        self._send_lines_with_ok(cmds)
        self.update_last_module(src_mod)
        current_mod_tracker = src_mod

        # --- Z OPTIMIZATION LOGIC FOR DEST ---
        use_optimized_z_dest = (current_mod_tracker in SMALL_VIAL_MODULES and dest_mod in SMALL_VIAL_MODULES)
        if use_optimized_z_dest:
            travel_z_dest = self.resolve_coords(0, 0, _4ML_RACK_CONFIG["Z_SAFE"])[2]
        else:
            travel_z_dest = global_safe_z

        # 3. Move to Dest (With Volatile Logic)
        cmds_move = []

        if is_volatile:
            current_e = e_loaded_pos

            # Phase A: Move Up to Travel Z (Aspirating)
            dist_z_up = abs(travel_z_dest - src_safe_z)
            time_z_up = dist_z_up / JOG_SPEED_Z
            drift_steps_up = (VOLATILE_DRIFT_RATE * STEPS_PER_UL) * (time_z_up / 60.0)

            # Since aspiration is negative E, we subtract positive steps (move E more negative)
            current_e -= drift_steps_up
            cmds_move.append(f"G1 Z{travel_z_dest:.2f} E{current_e:.3f} F{JOG_SPEED_Z}")

            # Phase B: Move XY to Dest (Aspirating)
            dist_xy = math.sqrt((dest_x - src_x) ** 2 + (dest_y - src_y) ** 2)
            time_xy = dist_xy / JOG_SPEED_XY
            drift_steps_xy = (VOLATILE_DRIFT_RATE * STEPS_PER_UL) * (time_xy / 60.0)
            current_e -= drift_steps_xy

            cmds_move.append(f"G1 X{dest_x:.2f} Y{dest_y:.2f} E{current_e:.3f} F{JOG_SPEED_XY}")

            # Phase C: Move Down to Dest Safe Z (Aspirating)
            dist_z_down = abs(travel_z_dest - dest_safe_z)
            time_z_down = dist_z_down / JOG_SPEED_Z
            drift_steps_down = (VOLATILE_DRIFT_RATE * STEPS_PER_UL) * (time_z_down / 60.0)
            current_e -= drift_steps_down

            cmds_move.append(f"G1 Z{dest_safe_z:.2f} E{current_e:.3f} F{JOG_SPEED_Z}")

            self.log_line(f"[VOLATILE] Total Drift Steps: {drift_steps_up + drift_steps_xy + drift_steps_down:.1f}")

        else:
            cmds_move.append(f"G0 Z{travel_z_dest:.2f} F{JOG_SPEED_Z}")
            cmds_move.append(f"G0 X{dest_x:.2f} Y{dest_y:.2f} F{JOG_SPEED_XY}")
            cmds_move.append(f"G0 Z{dest_safe_z:.2f} F{JOG_SPEED_Z}")

        self._send_lines_with_ok(cmds_move)
    ...
    def _perform_wash_cycle(self, wash_src_str, original_src_str, dest_str, vol, is_volatile, e_gap_pos, air_gap_ul,
        ...
        # --- FIX: Check if we are already ejected to avoid redundant ejection ---
        ...
        s_mod, s_x, s_y, s_safe_z, _, s_disp_z = self.get_coords_from_combo(original_src_str)
        ...
        cmds_src.append(f"G1 E{e_blowout:.3f} F{PIP_SPEED}")
        ...
        cfg = config_map.get(s_mod)
        if cfg:
            mix_cmds, _ = self._get_mix_commands(cfg)
            self.log_line("[WASH] Mixing in source vial...")
            self._send_lines_with_ok(mix_cmds)

        # --- NEW LOGIC: Transfer Mixed Wash to Dest ---
        self.log_line("[WASH] Transferring mixed wash liquid to destination...")
        transfer_vol = min(vol + 100.0, 800.0)

        # Perform transfer. Pass current state (s_mod) and get final state (dest_mod)
        current_mod_tracker = self._perform_single_transfer(
            original_src_str, dest_str, transfer_vol, is_volatile, e_gap_pos, air_gap_ul,
            start_module=current_mod_tracker
        )
    ...
    def combine_fractions_sequence(self):
        ...
        for task in tasks:
            ...
            if wash_vol > 0:
                ...
                # 2. Pick a fresh tip for the wash phase
                tip_key = self._find_next_available_tip()
                if not tip_key:
                    messagebox.showerror("No Tips", f"Ran out of tips for wash at Line {line_num}.")
                    return
                self.log_line(f"[COMBINE] Line {line_num}: Picking Wash Tip {tip_key}...")
                self._send_lines_with_ok(self._get_pick_tip_commands(tip_key, start_module=current_sim_module))
                self.tip_inventory[tip_key] = False
                self.update_last_module("TIPS")
                current_sim_module = "TIPS"
                ...
                for cycle in range(wash_times):
                    ...
                    for well in wells:
                        ...
                        # Dispense Wash into Well
                        cmds_well.append(f"G0 Z{plate_asp_z:.2f} F{JOG_SPEED_Z}")  # Dispense low
                ...
                # 4. Eject Wash Tip
                self.log_line(f"[COMBINE] Line {line_num}: Ejecting wash tip...")
                self._send_lines_with_ok(self._get_eject_tip_commands())
                self.update_last_module("EJECT")
                current_sim_module = "EJECT"
