...
class LiquidHandlerApp:
    ...
    def _build_transfer_liquid_tab(self, parent):
        ...
        for i in range(8):
            ...
            cb_mod = ttk.Combobox(row_frame, textvariable=row_vars["src_mod"], values=module_names, width=12,
                                  state="readonly")
            ...
            cb_mod.bind("<<ComboboxSelected>>",
                        lambda e, m=row_vars["src_mod"], p=cb_pos,
                               v=row_vars["src_pos"]: self._update_source_pos_options(m, p, v))

            # Initialize positions for the default value
            self._update_source_pos_options(row_vars["src_mod"], cb_pos, row_vars["src_pos"])

            # Dest Vial
            ttk.Combobox(row_frame, textvariable=row_vars["dest"], values=dest_options, width=15,
            ...
            # Wash Source
            ttk.Combobox(row_frame, textvariable=row_vars["wash_src"], values=dest_options, width=12,
                         state="readonly").pack(side="left", padx=2)
